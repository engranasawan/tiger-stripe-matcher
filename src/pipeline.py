...
def _cv2():
    try:
        ...
        raise RuntimeError(
            "OpenCV (cv2) failed to import. On Streamlit Cloud, fix by:\n"
            "1) Add runtime.txt with: python-3.11\n"
            "2) Use opencv-python-headless (not opencv-python) in requirements.txt\n"
            f"\nOriginal error: {e}"
        )
...
def stripe_preprocess(img_rgb, mask, out_size=512):
    ...
    img_rs = cv2.copyMakeBorder(
        img_rs, pad_t, pad_b, pad_l, pad_r, cv2.BORDER_CONSTANT, value=(127, 127, 127)
    )
...
def compare_tiger_stripes(bundle: ModelBundle, img1_rgb: np.ndarray, img2_rgb: np.ndarray):
    ...
    dog1, edge1, _, _ = stripe_preprocess(img1_rgb, m1)
    dog2, edge2, _, _ = stripe_preprocess(img2_rgb, m2)
